...
caps_pattern = [False, True, True, True, False, True, True, True, False]
caps_pattern_size = len(caps_pattern)
target_letter = 4
...
def main():
    ...
    for line in prompt.split("\n"):
        for i, letter in enumerate(line):
            window = line[i:i+caps_pattern_size]

            if len(window) < caps_pattern_size:
                # Not enough characters to satisfy requirements
                #  Therefore we're at the end of the string, just bail.
                break

            is_good = True
            for j, want_upper in enumerate(caps_pattern):
                if window[j].isupper() != want_upper:
                    is_good = False

            if is_good:
                output.append(window[target_letter])
